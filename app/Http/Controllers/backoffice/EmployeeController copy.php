...
class EmployeeController extends Controller
{
    ...
    public function store(EmployeePostRequest $request)
    {
        try
        {
            ...
            // the datatable
            $encodedId    = $this->hashids->encode($insertResults->getAttribute('id'));
            $empNum       = $insertResults->getAttribute(Employee::f_EmpNo);

            $rowData = [
                'emp_num'       => $empNum,
                'emp_status'    => $insertResults->getAttribute(Employee::f_Status),
                'total_lates'   => 0,
                'total_leave'   => 0,
                'total_absents' => 0,
                'id'            => $encodedId,
                'empname'       => implode(' ', [
                    $insertResults->getAttribute(Employee::f_LastName).', ',
                    $insertResults->getAttribute(Employee::f_FirstName),
                    $insertResults->getAttribute(Employee::f_MiddleName),
                ])
            ];
            ...
            $email  = $insertResults->getAttribute(Employee::f_Email);
            ...
            if (!empty($email))
            {
                ...
                $str_replace = [ 
                    $insertResults->getAttribute(Employee::f_FirstName), 
                    $insertResults->getAttribute('rawPinCode')
                ];
            ...
            $option_saveQR_localCopy = $inputs['option-save-qr'] == Constants::CHECKBOX_ON;
...
// [option-save-qr] => off,
    ...
    private function sendEmployeeQR()
    {
