...
function initialize()
{
    ...
    autoSuggestSrcTarget = $(teachers_datasetTable).data('src-emp-ids');
...
function bindEmployeeSuggestions()
{
    $.ajax({
        url: autoSuggestSrcTarget,
        type: 'POST',
        data: {
            '_token': csrfToken,
        },
        success: function(response) 
        {
            ...
            if (response)
            {
                ...
                employeesMap = response.reduce(function(map, employee)
                {
                    map[employee.idNo] = employee.name;
                    suggestions.push(employee.idNo);
                    
                    return map;
                }, {});
            }

            set_autocomplete('input-id-no', suggestions, start_at_letters=1);
        }
    });
}
